...
# Bez argumentow: clean package. Mozna podac wlasne cele, np. ./build-server.sh clean test
set -euo pipefail
...
JAVA_HOME="$JAVA_HOME21" exec "$MAVEN_HOME9/bin/mvn" -B -f "$ROOT/server/pom.xml" "$@"
